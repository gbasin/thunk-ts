...
| A2 | Per-session tokens generated lazily on first access, not at session creation | Backward-compatible with existing sessions; simpler than migrating old sessions | dw about backwards compat
...
- think through what the interaction will look like if i'm using claude code through a tmux/mosh session over tailscale from my iphone into my macbook (so claude code is running on my mac) to kick off thunk work with the plugin command. will everythign work? what will the URL look like, when will cc trigger the wait/continue commands and how, etc.
